...
export default function ArticleEditor() {
  ...
  const clearDraft = () => { setArticle(EMPTY_ARTICLE); setStatus('New blank article ready.') }
  ...
  async function uploadImage(event) {
    ...
    const safeName = file.name.replace(/[^a-z0-9.-]/gi, '-').replace(/-+/g, '-').toLowerCase()
    const path = `${user.id}/${Date.now()}-${safeName || `image.${extension}`}`
    ...
    setStatus('Image uploaded. Click Save changes.')
    setUploading(false)
  }
  ...
  async function saveDraft(event) {
    ...
    setArticle((current) => ({ ...current, imageUrl: data.hero_image_url || '', imageCaption: data.hero_image_caption || '' }))
    setStatus('Saved successfully. The database image is now the source of truth.')
  }
